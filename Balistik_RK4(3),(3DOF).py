...
try:
    ...
    env = {
        "g": 9.80665,        # Yerçekimi
        "T": 35.0,           # Sıcaklık (C)
        "P_basinc": 940.0, # Basınç (hPa/mbar)
        "N": 50.0,           # Nem (%)
        "enlem": 38.6,       # Enlem
        "pusula": 0.0,       # Atış yönü
        "rüzgar_dik": 0.0,
        "rüzgar_yan": 0.0,
        "namlu_yüksekligi": 1.5,
        "derece":35.0  #atış açısı
    }
    ...
    def get_accelerations(state, env, bullet, t):
        ...
        az_cor = 2 * omega * vx * math.cos(phi) * math.sin(theta)
                ...
                #Spin drift hesabı
        if yiv_varligi == "evet":
            spin_yon_çarpani = 1.0 if helis_yonu == "sağ" else -1.0
            
            # Zaman bağımlılığı yerine, anlık hıza duyarlı mikro-ivme modeli:
            a_spin_y = spin_yon_çarpani * 0.0015 * (bullet['Sg'] + 1.2) * (v_anlik / 100.0)
        else:
    ...
    if x_son > 0:
        ...
        range_factor = 1.0 + (x_son / 2500) # 1000m'de %40 ek düzeltme payı bırakır
        mil_dikey = (dikey_fark_m / x_son) * 1000 * range_factor
        moa_dikey = mil_dikey * 3.438

        # 2. Yatay Sapma (Windage):
        # Sistemin karşı tarafa fırlamasını (overshoot) engelleyen sönümleme katsayısı
        if x_son<500:
            damping_factor = 0.95
        elif 500<x_son<700:
            damping_factor=0.90
        ...
        mil_yatay = ((-y_son / x_son) * 1000) * damping_factor
